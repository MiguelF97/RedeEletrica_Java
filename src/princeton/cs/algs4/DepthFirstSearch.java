...
import Projeto.CommunicationGraph;
...
public class DepthFirstSearch {
     ...
     */
    public DepthFirstSearch(CommunicationGraph G, int s) {
        marked = new boolean[G.V()];
        dfs(G, s);
    }

    // depth first search from v
    private void dfs(CommunicationGraph G, int v) {
        count++;
        marked[v] = true;
        for(int i=0;i<G.V();i++)
           {
    ...
    public static void main(String[] args) {
        In in = new In(args[0]);
        CommunicationGraph G = new CommunicationGraph(in);

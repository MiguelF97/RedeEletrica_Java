...
public class Date implements Comparable<Date> {
   ...
   /**
     ...
     */
    public Date(int month, int day, int year, int hour, int min, int sec) {
        if (!isValid(month, day, year, hour, min, sec)) throw new IllegalArgumentException("Invalid date");
        this.month = month;
        this.day   = day;
        this.year  = year;
        this.hour = hour;
    ...
    private static boolean isValid(int m, int d, int y, int h, int mi, int s) {
        if (m < 1 || m > 12)      return false;
        if (d < 1 || d > DAYS[m]) return false;
        if (h < 0 || h > 23)      return false;
        if (mi < 0 || m > 59)     return false;
     ...
     */
    public Date next() {
        if (isValid(month, day + 1, year, hour,min,sec))    return new Date(month, day + 1, year,hour,min,sec);
        else if (isValid(month + 1, 1, year,hour,min,sec)) return new Date(month + 1, 1, year,hour,min,sec);
        else                                  return new Date(1, 1, year + 1,hour,min,sec);
    }
    ...
    public boolean equals(Object x) {
        ...
        Date that = (Date) x;
        return (this.month == that.month) && (this.day == that.day) && (this.hour == that.hour) && (this.min == that.min) && (this.year == that.year) && (this.sec == that.sec);
    }
